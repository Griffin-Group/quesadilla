...
MODULE matrix_inversion
  ...
  double precision, DIMENSION (n,n), INTENT(out), OPTIONAL :: a_inv
  double precision, OPTIONAL, INTENT(out) :: da
  !
  ...
  double complex, OPTIONAL, DIMENSION (n,n), INTENT(OUT) :: a_inv
  double complex, OPTIONAL, INTENT(OUT) :: da
  !
  INTEGER :: info, lda, lwork
  ! info=0: inversion was successful
  ! lda   : leading dimension (the same as n)
